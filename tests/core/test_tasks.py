...
def a_failed_task_for_testing_overrides_severity(task: Task) -> Result:
    return Result(
        host=task.host,
...
def sub_task_for_testing_overrides_severity(task: Task, fail_on: list[str] | None = None) -> None:
    task.run(
        a_task_for_testing,
        fail_on=fail_on,
...
class Test:
    def test_task(self, nornir: Nornir) -> None:
        result = nornir.run(a_task_for_testing)
        assert result
        for h, r in result.items():
            assert r.stdout.strip() == h

    def test_sub_task(self, nornir: Nornir) -> None:
    ...
    def test_run_on(self, nornir: Nornir) -> None:
    ...
    def test_dry_run(self, nornir: Nornir) -> None:
    ...
    def test_subtask_exception_no_capture(self, nornir: Nornir) -> None:
    ...
    def test_subtask_exception_capture(self, nornir: Nornir) -> None:
        host = nornir.filter(name="dev1.group_1")

...
def configure_task(task):
    r = task.run(
        text.template_file, template="base.j2", path="../templates/{brigade_nos}"
    )
    task.host["config"] = r.result

    r = task.run(data.load_yaml, file="../extra_data/{host}/l3.yaml")
    task.host["l3"] = r.result

    r = task.run(
        text.template_file, template="interfaces.j2", path="../templates/{brigade_nos}"
    )
    task.host["config"] += r.result

    r = task.run(
        text.template_file, template="routing.j2", path="../templates/{brigade_nos}"
    )
    task.host["config"] += r.result

    r = task.run(
        text.template_file, template="{role}.j2", path="../templates/{brigade_nos}"
    )
    task.host["config"] += r.result

    return task.run(
        networking.napalm_configure, replace=False, configuration=task.host["config"]
    )
...
def configure(ctx, validate, backup, backup_path, rollback):
    ...
    if validate:
        ...
        if r.failed and rollback:
            ...
            filtered.run(text.print_result, num_workers=1, data=r)
    import pdb

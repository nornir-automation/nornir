...
r_rgba = re.compile(
    fr"\s*rgba\(\s*{_r_255}{_r_comma}{_r_255}{_r_comma}{_r_255}{_r_comma}{_r_alpha}\s*\)\s*"
)
...
class Color(Representation):
    ...
    def as_hsl(self) -> str:
        ...
        if self._rgba.alpha is None:
            ...
            return (
                f"hsl({h * 360:0.0f}, {s * 100:0.0f}%, {li * 100:0.0f}%, {round(a, 2)})"
            )
...
def parse_hsl(
    h: str, h_units: str, s: str, l: str, alpha: Optional[float] = None
) -> RGBA:
    """
